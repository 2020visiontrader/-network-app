...
const DiscoveryScreen = ({ navigation }) => {
  ...
  const loadExistingConnections = async () => {
    try {
      ...
      setExistingConnections(new Map([
  ...
  const sendConnectionRequest = async (founderId) => {
    try {
      const connectionStatus = existingConnections.get(founderId);
      ...
      setExistingConnections(prev => new Map(prev).set(founderId, 'pending'));
  ...
  const renderFounder = ({ item: founder }) => {
    const connectionStatus = existingConnections.get(founder.id);
