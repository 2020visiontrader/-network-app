...
# Expo SDK 51 QR Code Generator for Expo Go Client 2.31.2
...
echo -e "${BLUE}=== Expo SDK 51 QR Code Generator for Expo Go v2.31.2 (with Logs) ===${NC}"
...
# Try LAN mode first, as it's often more stable than tunnel
echo -e "${YELLOW}Starting Expo in LAN mode with detailed logging...${NC}"
echo -e "${YELLOW}When the QR code appears, scan it with Expo Go v2.31.2${NC}"
echo -e "${YELLOW}If LAN mode doesn't work, press Ctrl+C and run with '--tunnel' option${NC}"
echo -e "${GREEN}Detailed logs will be saved to ${LOG_FILE}${NC}"
echo ""

echo "===== STARTING EXPO =====" >> "$LOG_FILE"
# Run Expo with detailed logging and a specific port to avoid conflicts
EXPO_DEBUG=1 EXPO_NO_DOCTOR=1 npx expo start --lan --no-dev --port 8086
